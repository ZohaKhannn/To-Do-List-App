# To-Do List App

A simple to-do list web app built with HTML, CSS, and JavaScript, with a custom light/dark theme toggle styled like a day-to-night switch (sun and mountains on one side, clouds, stars, and a cratered moon on the other).

## Features
- Add new tasks to the list
- Mark tasks as complete
- Delete tasks
- Custom animated theme toggle switch (light/dark mode)
- Custom SVG icon on the "Add" button

## Built with
- HTML
- CSS
- JavaScript

## What I learned / what was tricky
Building the custom toggle animation, handling task state in JS, structuring CS was bit harder for me as it was my first project.

## How to run it
1. Clone or download this repository
2. Open `index.html` in your browser
3. Start adding tasks
